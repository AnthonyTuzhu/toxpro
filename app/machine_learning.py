...
from app.stats import class_scoring, regressive_scoring, get_class_stats, get_regressive_stats
...
REGRESSOR_ALGORITHMS = [
    ...
    ('kNN', KNeighborsRegressor(metric='euclidean'), {'kNN__n_neighbors': [1, 3, 5, 10 , 20],
                                                       'kNN__weights': ['uniform', 'distance']}),
    ('SVM', SVR(), {'SVM__kernel': ['linear', 'rbf', 'poly'],
                                     'SVM__gamma': [1e-2, 1e-3],
                                     'SVM__C': [0.1, 1, 10]}),
...
def build_qsar_model(X: pd.DataFrame, y: pd.Series, alg: str, scale=True):
    ...
    five_fold_stats = get_regressive_stats(None, y, cv_predictions[1])
...
def build_qsar_regressive_model(X: pd.DataFrame, y: pd.Series, alg: str, scale=True):
    """ build a QSAR regressive model """
    cv = model_selection.StratifiedKFold(shuffle=True, n_splits=5, random_state=seed)
    ...
    return regressive_model, cv_predictions, five_fold_stats

if __name__ == '__main__':
    print(CLASSIFIER_ALGORITHMS_DICT)
    print(REGRESSOR_ALGORITHMS_DICT)